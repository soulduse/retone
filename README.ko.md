...
**이미 결제 중인 구독을 그대로 활용하세요.** Retone은 공식 `claude` / `codex` CLI를 헤드리스 서브프로세스로 감싸 실행하므로, Claude Pro/Max 또는 ChatGPT Plus/Pro 구독으로 리라이팅이 이뤄집니다 — OAuth 토큰 추출도, 서드파티 서버도 없습니다. 직접 준비한 API 키(Anthropic / OpenAI / Gemini)도 지원합니다.
...
2. 프로바이더/모델 선택 (Claude CLI: sonnet/haiku/opus, Codex: gpt-5.5, …)
